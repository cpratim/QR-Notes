...
\usepackage[noend]{algpseudocode} % 'noend' removes 'end' statements for compactness
...
\begin{enumerate}
    ...
    \begin{algorithm}
        ...
        \State Shuffle \( D \) randomly and partition into \( k \) equal-sized folds \( \{F_1, F_2, \dots, F_k\} \) \Comment{Adjust if \( |D| \) not divisible by \( k \)}
        \State Initialize scores list \( S \gets [] \)
        \For{\( i = 1 \) to \( k \)} \Comment{Iterate over each fold}
            \State Define test set \( D_{\text{test}} \gets F_i \)
            \State Define training set \( D_{\text{train}} \gets D \setminus F_i \) \Comment{Union of all other folds}
        ...
        \EndFor
        \State Compute \( S_{\text{avg}} \gets \frac{1}{k} \sum_{i=1}^{k} s_i \) \Comment{Average score across folds}
...
\begin{enumerate}
    ...
    \begin{algorithm}
        ...
        \State Initialize \( S_{\text{best}} \gets -\infty \) \Comment{Assuming higher score is better; adjust for minimization}
        \State Initialize \( P_{\text{best}} \gets \emptyset \)
        \State Generate all possible combinations \( C \) of hyperparameters from \( P_1 \times P_2 \times \dots \times P_k \) \Comment{Cartesian product}
        \For{\( p \in C \)} \Comment{Iterate over each combination}
            \State Train model \( M \) on \( D_{\text{train}} \) using hyperparameters \( p \)
